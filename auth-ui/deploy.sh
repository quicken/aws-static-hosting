...
aws s3 sync dist/ s3://$AWS_S3_BUCKET --delete $PROFILE_FLAG

echo "Invalidating CloudFront distribution: $AWS_CLOUDFRONT_DISTRIBUTION_ID"
aws cloudfront create-invalidation --distribution-id $AWS_CLOUDFRONT_DISTRIBUTION_ID --paths "/*" $PROFILE_FLAG

echo "Deployment complete!"
echo "Your app should be available at your CloudFront domain shortly."
